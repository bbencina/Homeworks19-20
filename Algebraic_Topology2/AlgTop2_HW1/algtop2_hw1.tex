...
%	\]
%	Writting out equations we get $\alpha_{03} = -\alpha_{01}$, $\alpha_{12} = \alpha_{01}$ and $\alpha_{23} = \alpha_{12} = \alpha_{01}$, so $(\alpha_{ij})_{i<j} = (\alpha_{01}, -\alpha_{01}, \alpha_{01}, \alpha_{01})$, so $\ker\Phi_*$ is $1$-dimensional and thus isomorphic to $\Z$. This proves $H_1(\S^1) \iso \Z$ as previously known.
%	
